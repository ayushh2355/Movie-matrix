...
export default function BookingSidebar({
  ...
  const errorClasses = "bg-red-950/40 text-red-400 border-red-900/60 cursor-not-allowed shadow-none";

  const buttonClasses = `${baseClasses} ${
    isOverLimit ? errorClasses : canBook ? activeClasses : disabledClasses
  }`;
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-5 border-t border-slate-800/60 pt-5">
          {selectedSeats.length > 0 ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest select-none">
                  Ticket Details
                </h3>
              ...
              {isOverLimit && (
                <div className="flex items-center gap-2 bg-red-950/40 border border-red-900/60 rounded-lg px-3 py-2">
                  <span className="text-red-400 text-xs">⚠</span>
                  <p className="text-[11px] text-red-400 font-semibold select-none">
                    Max {MAX_SEATS} seats allowed. Please deselect {selectedSeats.length - MAX_SEATS} seat{selectedSeats.length - MAX_SEATS > 1 ? "s" : ""}.
                  </p>
                </div>
              )}
              ...
              <div className="pt-2">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2 select-none">
                  Selected Seats
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {selectedSeatDetails.map((seat: Seat) => {
                    ...
                    const badge =
                      t === "Bronze" ? "bg-zinc-900/60 text-zinc-300 border-zinc-800" :
                      t === "Silver" ? "bg-slate-900/60 text-slate-300 border-slate-800" :
                                       "bg-yellow-950/40 text-yellow-300 border-yellow-900/40";
                    return (
                      <span key={seat.id} className={`text-[9px] font-black px-2 py-0.5 rounded border select-none ${badge}`}>
                        {seat.row}{seat.col}
                      </span>
                    );
                  })}
          ...
          <div className="border-t border-slate-800/80 pt-4 mt-6 flex justify-between items-center">
            ...
            <span className="text-2xl font-black text-amber-400 drop-shadow-[0_0_8px_rgba(245,158,11,0.25)]">
              ₹{totalPrice}
            </span>
          </div>
        </div>
