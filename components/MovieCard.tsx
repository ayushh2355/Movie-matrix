...
export default function MovieCard({ movie }: { movie: Movie }) {
  ...
  return (
    <div className="flex flex-col group">
      ...
      <Link
        href={`/movie/${movie.id}`}
        className="relative aspect-[3/4] w-full rounded-2xl overflow-hidden shadow-lg border border-slate-900 hover:border-slate-800/80 transition-all select-none"
      >
        <div
      ...
      <div className="mt-3.5 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-start justify-between gap-2">
            <Link
              href={`/movie/${movie.id}`}
              className="text-sm font-bold text-slate-100 group-hover:text-amber-400 transition-colors line-clamp-1"
            >
              {movie.title}
            </Link>
